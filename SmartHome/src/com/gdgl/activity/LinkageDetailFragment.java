...
public class LinkageDetailFragment extends Fragment implements ChangeFragment,
		AddChecked {
		...
		if (linkage_type == 2) {
			...
			if (linkageAct.getType().equals("4")
				...
				actDevices = DataUtil.getDeviceModelByIeee(
						linkageAct.getIeee(), mDataHelper,
						mDataHelper.getSQLiteDatabase());
			}
	...
	private void setListeners() {
		...
		devices_choice_lay.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				mLinkageDevicesAddListAdapter = mLinkageAddFragment.new LinkageDevicesAddListAdapter(
						mTrgAddList);
				...
				MyApplicationFragment.getInstance().addFragment(
						mLinkageAddFragment);
			}
		});
	...
	private String getTrgcnd() {
		...
		if (trgDevices.getmModelId().indexOf((DataHelper.Magnetic_Window)) == 0) { // 窗磁Z311A
			...
			// trgcnd += "Burglar@eq@0";
			trgcnd += "alarm@eq@0";
		} else if (trgDevices.getmModelId().indexOf(
				(DataHelper.Smoke_Detectors)) == 0) { // 烟雾感应器ZA01A
			// trgcnd += "Fire@eq@0";
			trgcnd += "alarm@eq@0";
		} else if (trgDevices.getmModelId().indexOf(
				(DataHelper.Indoor_temperature_sensor)) == 0) { // 温湿度感应器Z711
			if (tem_hum == 0)
				trgcnd += "temp@" + SPINNER_SIGN_DATA[temp] + "@"
						+ devices_data_edit.getText().toString();
			else
				trgcnd += "hum@" + SPINNER_SIGN_DATA[temp] + "@"
						+ devices_data_edit.getText().toString();
		} else if (trgDevices.getmDeviceId() < 0) {
			trgcnd += "alarm@eq@0";
		}
		return trgcnd;
	}
	...
	private String getLnkact() {
		...
		if (actDevices.getmDevicePriority() == 1000) {
			...
			// lnkact += 1;
			// }else
			// if(actDevices.getmModelId().indexOf((DataHelper.Motion_Sensor))
			// == 0){ //动作感应器ZB11A
			// lnkact += 1;
			// }else
			// if(actDevices.getmModelId().indexOf((DataHelper.Smoke_Detectors))
			// == 0){ //烟雾感应器ZA01A
			// lnkact += 1;
			// }else if(actDevices.getmModelId().indexOf((DataHelper.Siren)) ==
			// 0){ //警报器Z602A
			// lnkact += 1;
			// }else
			// if(actDevices.getmModelId().indexOf((DataHelper.Indoor_temperature_sensor))
			// == 0){ //温湿度感应器Z711
			// lnkact += 1;
			// }else
			// if(actDevices.getmModelId().indexOf((DataHelper.Power_detect_wall))
			// == 0){ //电能检测墙面插座Z816H
			// lnkact += 3;
			// }else
			// if(actDevices.getmModelId().indexOf((DataHelper.Power_detect_socket))
			// == 0){ //电能检测插座Z809A=====王晓飞
			// lnkact += 3;
			// }else
			// if(actDevices.getmModelId().indexOf((DataHelper.One_key_operator))
			// == 0){ //物联网网关
			// lnkact += 2;
			// }else { //
			// lnkact += 3;
			// }
			lnkact += DataUtil.getTimingSceneActionParamType(actDevices
					.getmDeviceId());
			lnkact += ":" + actDevices.getmIeee() + "-" + actDevices.getmEP()
					+ "-" + onoff;
		}
		return lnkact;
	}
	...
	private void initTrgAddFragmentDevicesList() {
		...
		List<DevicesModel> mAddList = mDataHelper.queryForDevicesList(db,
		...
		for (DevicesModel mDevices : mAddList) {
			if (mDevices.getmModelId().indexOf((DataHelper.Magnetic_Window)) == 0) { // 窗磁Z311A
				mTrgAddList.add(mDevices);
					...
					(DataHelper.Motion_Sensor)) == 0) { // 动作感应器ZB11A
				mTrgAddList.add(mDevices);
			} else if (mDevices.getmModelId().indexOf(
					(DataHelper.Smoke_Detectors)) == 0) { // 烟雾感应器ZA01A
				mTrgAddList.add(mDevices);
				...
				mTrgAddList.add(mDevices);
					...
					DataHelper.RF_Magnetic_Door) == 0
					|| mDevices.getmModelId().indexOf(
							DataHelper.RF_Magnetic_Door_Roll) == 0
					|| mDevices.getmModelId().indexOf(
							DataHelper.RF_Infrared_Motion_Sensor) == 0
					|| mDevices.getmModelId().indexOf(
							DataHelper.RF_Smoke_Detectors) == 0
					|| mDevices.getmModelId().indexOf(
	...
	private void initActAddFragmentDevicesList() {
		...
		for (DevicesModel mDevices : mAddList) {// ----王晓飞---添加||power_detect_socket
			if (mDevices.getmModelId().indexOf((DataHelper.Power_detect_wall)) == 0
					|| mDevices.getmModelId().indexOf(
							(DataHelper.Power_detect_socket)) == 0) { // 电能检测墙面插座Z816H
				mActAddList.add(mDevices);
			}
			// else if(mDevices.getmModelId().indexOf((DataHelper.Siren)) == 0){
			// //警报器Z602A
			// mActAddList.add(mDevices);
			// }
		}
	...
	private void updateActDevices() {
		if (actDevices.getmDevicePriority() == 1000) {
			...
			act_img.setBackgroundResource(DataUtil.getDefaultDevicesSmallIcon(
					actDevices.getmDeviceId(), actDevices.getmModelId().trim()));
		}
	...
	private void updatetrgDevices() {
		...
		if (trgDevices.getmModelId().indexOf(
